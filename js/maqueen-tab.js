...
(function () {
  ...
  function fireDrive(dataL, dataR, opts) {
    ...
    if (dataL === 0 && dataR === 0) {
      // Drain any pending coalesced M: so it can't race the STOP.
      // M: is rate-limited to 8 Hz; without this drain a queued M:L,R
      // flush fires ~125 ms after STOP and re-starts the motors.
  ...
  function initDrive() {
    ...
    // Home pin — draggable replay origin marker on the map.
    mqHomePin.init();
    // Pen mode.
    mqPen.init();
    // Click the drive mascot SVG → reset odometry heading to 0.
    const mascotSvg = document.getElementById('mqMascot');
    if (mascotSvg) {
      mascotSvg.style.cursor = 'pointer';
      mascotSvg.title = 'Click to reset heading to 0°';
      mascotSvg.addEventListener('click', () => {
        try { mqOdometry.resetHeading(); } catch {}
      });
    }
    ...
    // Replay scrubber (range input) — set start point.
    const macroScrubber = document.getElementById('mqMacroScrubber');
    if (macroScrubber) {
      macroScrubber.addEventListener('input', () => {
        mqMacro.setScrub(macroScrubber.value / 1000);
      });
    }
    // Click on the map SVG — two modes:
    //   draw mode (pen): add waypoint
    //   otherwise: set replay start point (if recording exists)
    const odoSvg = document.getElementById('mqOdoSvg');
    if (odoSvg) {
      odoSvg.addEventListener('click', e => {
        const ctm = odoSvg.getScreenCTM();
        if (!ctm) return;
        const pt = odoSvg.createSVGPoint();
        pt.x = e.clientX; pt.y = e.clientY;
        const svgPt = pt.matrixTransform(ctm.inverse());
        if (mqPen.isDrawMode()) {
          const s = mqOdometry.getScale();
          mqPen.addWaypoint(svgPt.x / s, -svgPt.y / s);
          return;
        }
        if (!mqMacro.canScrub()) return;
        const frac = mqMacro.nearestFrameFrac(svgPt.x, svgPt.y);
        if (frac === null) return;
        mqMacro.setScrub(frac);
        if (macroScrubber) macroScrubber.value = Math.round(frac * 1000);
      });
    }
    // Macro import / export buttons.
    const macroExport = document.getElementById('mqMacroExport');
    if (macroExport) macroExport.addEventListener('click', () => mqMacro.exportMacro());
    const macroImportFile = document.getElementById('mqMacroImportFile');
    const macroImport = document.getElementById('mqMacroImport');
    if (macroImport && macroImportFile) {
      macroImport.addEventListener('click', () => macroImportFile.click());
      macroImportFile.addEventListener('change', e => {
        mqMacro.importMacro(e.target.files[0]);
        macroImportFile.value = '';   // reset so re-picking same file fires change again
      });
    }
  ...
  function initServos() {
    ...
    function setAngle(port, angle, opts) {
      ...
      // visual — 360° mode: animate continuous spin at the implied speed.
      // angle 90° = stopped (speed 0), 0° = full reverse, 180° = full forward.
      // 180° mode: just snap the dial to the physical position.
      if (mode[port] === '360') {
        const spd = (angle - 90) / 0.9;   // maps 0-180° → -100..+100
        startContinuousSpin(port, spd);
      } else {
    ...
    function startSweep(port) {
      ...
      // Wire the SWP: subscriber now (deferred from initServos() time
      // because sweep-mode.js loads after maqueen-tab.js).
      ensureSwpSubscribed();

      // In 360° mode the dial may be spinning via startContinuousSpin —
      // stop that timer so the sweep animation exclusively owns the dial.
      ...
      // Kill CSS snap-transition so 30 Hz setAttribute calls aren't smoothed.
      const sweepDial = port === 1 ? dial1 : dial2;
      if (sweepDial) sweepDial.style.transition = 'none';

      // ===== SHARED local-visual animation loop =====
      // Runs in BOTH modes:
      //   • Browser mode — this IS the servo driver (sends SRV: every 85 ms)
      //   • Firmware mode — purely visual; robot drives the real servo.
      //     SWP: push-backs (via ensureSwpSubscribed) will correct any drift,
      //     but the local loop guarantees the UI animates immediately even
      //     before the first SWP: arrives.
      const VISUAL_HZ = 30;
      ...
      window.bleScheduler.animate(`servo-sweep-${port}`, t => {
        ...
        // connected. Returning null avoids calling dispatch(), which would
        // trigger bleScheduler.stopAll() on every tick while disconnected,
        // killing the animation loop and freezing the slider/dial.
        if (useFirmware || !window.bleScheduler.isConnected()) return null;
        // === BLE SEND GATE (browser mode, connected only) ===
    ...
    function stopSweep(port) {
      ...
      // In 360° mode, sweep returning ownership of the dial means the servo
      // is now stopped (90° = neutral). Resume the spin sim at speed 0 so
      // the dial stays put (no ghost spinning after sweep ends).
      if (mode[port] === '360') startContinuousSpin(port, 0);
      paintSweepButton(port);
    }
    ...
    //
    // IMPORTANT: maqueen-tab.js loads before sweep-mode.js in the <script>
    // list, so window.mqSweepMode is undefined here. Registering the handler
    // eagerly at initServos() time silently no-ops. We subscribe LAZILY on
    // the first startSweep() call — by then all defer scripts are loaded.
    let swpSubscribed = false;
    function ensureSwpSubscribed() {
      if (swpSubscribed) return;
      if (!window.mqSweepMode || !window.mqSweepMode.onSWP) return;
      swpSubscribed = true;
    ...
    function setMode(port, m) {
      ...
      // Both 360° and 180° modes share identical UI — same slider range,
      // same degree readout, same 0°/90°/180°/Sweep buttons. The only
      // difference is how the physical servo responds: a continuous-rotation
      // servo interprets the same 0°-180° pulse widths as direction+speed
      // instead of position. Kids observe that contrast directly.
      ...
      // Transition state: snap for 180°, remove for 360° (spin loop owns the dial)
      if (m === '360') {
        if (dial) dial.style.transition = 'none';
      } else {
        if (dial) dial.style.transition = 'transform 0.18s ease-out';
        // Cancel any spin left over from 360° session
        if (spinTimers[port]) { cancelAnimationFrame(spinTimers[port]); spinTimers[port] = null; }
      }
      if (quickEl) {
        quickEl.innerHTML =
      ...
      if (m === '360') {
        startContinuousSpin(port, 0);   // stopped — dial stays put, transition already removed
      } else {
        rotateDial(port, 90);
      }
    ...
    // Both modes use the same 0-180° slider → always dispatch as angle.
    // A 360° servo interprets those pulse widths as speed, not position,
    // but the command sent (SRV:port,angle) is identical.
    ...
    function rewireQuickButtons() {
      document.querySelectorAll('.mq-servo-quick').forEach(b => {
        b.addEventListener('click', () => {
          const port = +b.dataset.port;
          if (b.dataset.angle != null) setAngle(port, +b.dataset.angle);
        });
      });
    }
  ...
  // -------- 📍 HOME PIN — draggable replay origin marker --------
  // The pin lives in the odometry SVG. Drag it to declare "this is
  // where the robot sits when I hit Replay." reset() will start the
  // odometry from that position so the orange replay trail lines up
  // with the green recorded trail.
  const mqHomePin = (function () {
    // Pin position stored in ODO WORLD coordinates (cm).
    // SVG display = worldX * scale, -worldY * scale — updated each render frame.
    let worldX = 0, worldY = 0;

    // Return the world-space position for use by reset()
    function getSvgPos() { return { x: worldX, y: worldY }; }

    function init() {
      const pin = document.getElementById('mqHomePinGroup');
      const svg = document.getElementById('mqOdoSvg');
      if (!pin || !svg) return;

      // Stamp world coords onto the DOM element so render() can reposition it
      pin._worldX = worldX;
      pin._worldY = worldY;

      let dragging = false, startMX, startMY, startWX, startWY;
      ...
      function onDown(e) {
        ...
        startMX = c.clientX; startMY = c.clientY;
        startWX = worldX;    startWY = worldY;
      ...
      function onMove(e) {
        ...
        e.preventDefault();
        const c  = e.touches ? e.touches[0] : e;
        const cur = toSvgCoords(c.clientX, c.clientY);
        const ori = toSvgCoords(startMX, startMY);
        // SVG delta → world delta: divide by current scale; flip y
        const scale = (typeof mqOdometry !== 'undefined') ? mqOdometry.getScale() : 1;
        worldX = startWX + (cur.x - ori.x) / scale;
        worldY = startWY - (cur.y - ori.y) / scale;   // SVG y is down, world y is up
        pin._worldX = worldX;
        pin._worldY = worldY;
        // Immediate visual update (render() will keep it synced on rAF)
        pin.setAttribute('transform',
          `translate(${(worldX * scale).toFixed(1)} ${(-worldY * scale).toFixed(1)})`);
      }
      function onUp() {
        if (!dragging) return;
        dragging = false;
      ...
      pin.addEventListener('mousedown',  onDown, { passive: false });
    ...
    return { init, getSvgPos };
  })();

  function getHomePinSvgPos() { return mqHomePin.getSvgPos(); }
  ...
  const mqMacro = (function () {
    ...
    let playStartFrac = 0;              // 0.0–1.0: fraction of recording to start from
    ...
    function paint() {
      ...
      const exportBtn = document.getElementById('mqMacroExport');
      if (exportBtn) exportBtn.disabled = frames.length === 0 || state === 'recording';
      if (playBtn) {
      ...
      // SVG cursor: crosshair when clicking the trail sets the start point
      const svg = document.getElementById('mqOdoSvg');
      if (svg) svg.style.cursor = (frames.length > 0 && state === 'idle') ? 'crosshair' : 'default';
      // Timeline scrubber — show when recording exists and not actively recording
      const timeline = document.getElementById('mqReplayTimeline');
      const scrubber = document.getElementById('mqMacroScrubber');
      const scrubLabel = document.getElementById('mqMacroScrubLabel');
      if (timeline) {
        const showTimeline = frames.length > 0 && state !== 'recording';
        timeline.style.display = showTimeline ? 'flex' : 'none';
        if (showTimeline && scrubber && scrubLabel) {
          const total = frames[frames.length - 1].t;
          if (state === 'playing') {
            // Advance scrubber to show playback progress
            const startT  = frames[playI > 0 ? playI - 1 : 0]?.t ?? 0;
            const elapsed = performance.now() - playStart;
            const curT    = Math.min(elapsed + (frames[0]?.t ?? 0), total);
            scrubber.value = Math.round((curT / total) * 1000);
            scrubLabel.textContent = fmtSec(curT) + ' / ' + fmtSec(total);
          } else {
            // Show chosen start point
            const startT = playStartFrac * total;
            scrubLabel.textContent = fmtSec(startT) + ' / ' + fmtSec(total);
          }
          // Move the SVG start dot along the trail
          updateStartDot(playStartFrac);
        }
      }
    }

    function updateStartDot(frac) {
      const dot = document.getElementById('mqReplayStartDot');
      if (!dot) return;
      if (frac <= 0 || frames.length === 0) { dot.setAttribute('display', 'none'); return; }
      // Find the frame at this fraction and use its recorded pose (x, y in cm).
      // The odometry SVG viewBox maps cm directly to SVG units, so no transform needed.
      const idx   = Math.min(Math.round(frac * (frames.length - 1)), frames.length - 1);
      const frame = frames[idx];
      if (frame.x == null) {
        // Old recording without pose data — fall back to trail-point approximation
        const trail = document.getElementById('mqOdoTrail');
        if (!trail) { dot.setAttribute('display', 'none'); return; }
        const pts = trail.points;
        if (!pts || pts.numberOfItems < 2) { dot.setAttribute('display', 'none'); return; }
        const pi = Math.round(frac * (pts.numberOfItems - 1));
        const pt = pts.getItem(Math.max(0, Math.min(pi, pts.numberOfItems - 1)));
        dot.setAttribute('display', '');
        dot.setAttribute('transform', `translate(${pt.x.toFixed(2)} ${pt.y.toFixed(2)})`);
      } else {
        // New recording — exact position from frame data.
        // Odometry uses (x = East, y = North, origin = 0,0).
        // SVG viewBox: x right, y DOWN — so SVG_y = -odo_y.
        dot.setAttribute('display', '');
        dot.setAttribute('transform', `translate(${frame.x.toFixed(2)} ${(-frame.y).toFixed(2)})`);
      }
    }
    ...
    // Trail colours (used by both recording reset and playback)
    const ODO_TRAIL_ORIG   = '#4ade80';  // live / recorded trail colour
    const ODO_TRAIL_REPLAY = '#f97316';  // orange while replaying

    function removeTrailSnapshot() {
      document.getElementById('mqOdoTrailSnap')?.remove();
    }

    // ---- Recording ------------------------------------------
    function startRec() {
      if (state !== 'idle') return;
      // New recording clears any leftover replay comparison and resets scrubber.
      removeTrailSnapshot();
      const trailEl = document.getElementById('mqOdoTrail');
      if (trailEl) trailEl.setAttribute('stroke', ODO_TRAIL_ORIG);
      playStartFrac = 0;
      const scrubber = document.getElementById('mqMacroScrubber');
    ...
    function recordCmd(L, R) {
      ...
      // Snapshot the robot's estimated position so we can later map a trail
      // click back to an exact frame (Option A: spatial frame lookup).
      const pose = (typeof mqOdometry !== 'undefined') ? mqOdometry.getPose() : null;
      frames.push({ t, L, R, x: pose ? +pose.x.toFixed(2) : null, y: pose ? +pose.y.toFixed(2) : null });
    }

    // ---- Playback -------------------------------------------
    // Freeze the current trail as a static green snapshot, rendered ON TOP
    // of mqOdoTrail (which turns orange). Result: green = original path,
    // orange = wherever the replay deviates / extends beyond the original.
    function snapshotTrail() {
      const trail = document.getElementById('mqOdoTrail');
      if (!trail) return;
      document.getElementById('mqOdoTrailSnap')?.remove();
      const snap = trail.cloneNode(false);       // copy element + attributes
      snap.id = 'mqOdoTrailSnap';
      snap.setAttribute('stroke', ODO_TRAIL_ORIG);
      snap.setAttribute('stroke-width', '2');    // slightly thicker so green shows
      snap.setAttribute('opacity', '1');
      snap.setAttribute('pointer-events', 'none');
      trail.parentNode.insertBefore(snap, trail.nextSibling);  // on top of trail
    }

    function play() {
      if (state !== 'idle' || frames.length === 0) return;
      state = 'playing';
      // Find the first frame at or after the chosen start fraction
      const total  = frames[frames.length - 1].t;
      const startT = playStartFrac * total;
      playI = frames.findIndex(f => f.t >= startT);
      if (playI < 0) playI = frames.length;     // past the end → plays nothing
      // Shift playStart so that frames[playI].t fires immediately
      playStart = performance.now() - (frames[playI]?.t ?? total);
      snapshotTrail();                           // freeze original in green
      // Reset odometry to the HOME pin world position so the replay trail
      // starts from the same visual origin as the recorded trail.
      // The pin is stored in SVG space; convert to world (cm) via current scale:
      //   world_x =  svg_x / scale
      //   world_y = -svg_y / scale   (SVG y is flipped)
      if (typeof mqOdometry !== 'undefined') {
        const pin = getHomePinSvgPos();   // already in world coords (cm)
        mqOdometry.reset(pin.x, pin.y);
      }
      const trailEl = document.getElementById('mqOdoTrail');
      if (trailEl) trailEl.setAttribute('stroke', ODO_TRAIL_REPLAY); // live trail → orange
      // Hide start dot while playing
      const dot = document.getElementById('mqReplayStartDot');
    ...
    function stopPlay() {
      ...
      state = 'idle';
      // Brake: cancel the in-flight motor command so the robot actually stops.
      _ourCall = true;
      try { fireDrive(0, 0); } catch {} finally { _ourCall = false; }
      // Keep the green snapshot + orange replay trail visible so kids can
      // compare original path vs replay path. Cleared only on new recording.
      paint();
    }
    function cancel() {
      ...
      if (state === 'playing') {
        if (playTimer) { clearTimeout(playTimer); playTimer = null; }
        state = 'idle';
        // Keep comparison visible after cancel too — same rationale as stopPlay.
        paint();
      }
    }
    ...
    function setScrub(frac) {
      if (state === 'playing') return;
      playStartFrac = Math.max(0, Math.min(1, frac));
      paint();
    }
    // True when clicking the map to set a start point makes sense
    function canScrub() { return frames.length > 0 && state === 'idle'; }

    // Find the fraction (0–1) of the frame nearest to SVG click (svgX, svgY).
    // Uses stored pose {x, y} on each frame (Option A).
    // Falls back to trail polyline proximity for old recordings without pose data.
    // Returns null if nothing useful to snap to.
    function nearestFrameFrac(svgX, svgY) {
      if (frames.length === 0) return null;
      // Option A: frames have stored pose
      if (frames[0].x != null) {
        // Odometry: x = East (SVG x), y = North (SVG y = -odo_y)
        let bestIdx = 0, bestDist = Infinity;
        frames.forEach((f, i) => {
          const d = (f.x - svgX) ** 2 + (-f.y - svgY) ** 2;
          if (d < bestDist) { bestDist = d; bestIdx = i; }
        });
        return bestIdx / (frames.length - 1);
      }
      // Fallback: use the live trail polyline points proportionally
      const trail = document.getElementById('mqOdoTrail');
      if (!trail) return null;
      const pts = trail.points;
      if (!pts || pts.numberOfItems < 2) return null;
      let bestIdx = 0, bestDist = Infinity;
      ...
      return bestIdx / (pts.numberOfItems - 1);
    }
    ...
    function exportMacro() {
      ...
      a.href     = url;
      a.download = `maqueen-macro-${ts}.json`;
      a.click();
    ...
    function importMacro(file) {
      ...
      reader.onload = e => {
        try {
          ...
          // Validate shape: each frame must have numeric t, L, R
          if (!parsed.every(f => typeof f.t === 'number' && typeof f.L === 'number' && typeof f.R === 'number')) {
          ...
          alert('⚠️ Invalid macro file — expected [{t,L,R},…] JSON.');
        }
      };
    ...
    return {
      ...
      setScrub, canScrub, nearestFrameFrac,
  ...
  const mqOdometry = (function () {
    ...
    function render() {
      ...
      // Keep the HOME pin visually fixed in world space as scale changes
      try {
        const pin = document.getElementById('mqHomePinGroup');
        if (pin && pin._worldX != null) {
          pin.setAttribute('transform',
            `translate(${(pin._worldX * scale).toFixed(1)} ${(-pin._worldY * scale).toFixed(1)})`);
        }
      } catch {}
      ...
      // Pen mode — re-render turtle preview + waypoints at current scale
      try { if (typeof mqPen !== 'undefined') { mqPen.renderPreview(); mqPen.renderWaypoints(); } } catch {}
    }

    function update(L, R) { vL = +L; vR = +R; }

    function reset(ox, oy) {
      x = (ox ?? 0); y = (oy ?? 0); theta = 0; totalDist = 0;
      trail.length = 0;
      obstacles.length = 0;
      const obstLayer = document.getElementById('mqOdoObstacles');
    ...
    return { update, recordAngle, recordDistance, reset, resetHeading, start,
             getTrail, getObstacles, getPose, getTotalDist,
             getScale: computeScale };
  })();
  ...
  function initLineFollow() {
    ...
    // Stop following automatically on disconnect. Use retry so this
    // registration survives any future script-order changes (same defensive
    // pattern as attachReplyListeners above).
    function wireFollowDisconnect() {
      if (!window.bleScheduler) { setTimeout(wireFollowDisconnect, 100); return; }
      window.bleScheduler.on('disconnected', stopFollow);
    }
    wireFollowDisconnect();
  }
  ...
  // -------- 🖊️ PEN MODE ----------------------------------------
  // Three modes unified under one toggle:
  // B — Drive-then-draw : thick trail + SVG export
  // C — Turtle graphics : command palette (Fwd/Back/Left/Right)
  // A — Draw-then-drive : click waypoints on map → auto-generate turtle cmds
  const mqPen = (function () {
    // Calibration (match firmware's VEL_SCALE + WHEELBASE from mqOdometry)
    const SPEED       = 200;
    const CM_PER_SEC  = 25;                           // at speed 200
    const WHEELBASE   = 9;
    const DEG_PER_SEC = (2 * SPEED * (CM_PER_SEC / SPEED) / WHEELBASE) * (180 / Math.PI); // ≈ 318

    let enabled   = false;
    let penMode   = 'turtle';        // 'turtle' | 'draw'
    let commands  = [];              // [{type, val}]
    let waypoints = [];              // [{x,y}] world coords (cm)
    let selCmd    = 'fwd';
    let executing = false;
    let execTimers = [];

    // ---- helpers --------------------------------------------------
    const odoScale = () => (typeof mqOdometry !== 'undefined') ? mqOdometry.getScale() : 1;
    const homePos  = () => (typeof getHomePinSvgPos !== 'undefined') ? getHomePinSvgPos() : {x:0,y:0};

    // ---- B: thick trail + SVG export ------------------------------
    function exportSvg() {
      const svg = document.getElementById('mqOdoSvg');
      if (!svg) return;
      const clone = svg.cloneNode(true);
      ['mqHomePinGroup','mqReplayStartDot','mqOdoRobot','mqWaypointDots',
       'mqOdoObstacles','mqOdoTarget'].forEach(id => clone.querySelector('#'+id)?.remove());
      const bg = document.createElementNS('http://www.w3.org/2000/svg','rect');
      Object.entries({x:'-100',y:'-100',width:'200',height:'200',fill:'white'})
            .forEach(([k,v]) => bg.setAttribute(k,v));
      clone.insertBefore(bg, clone.firstChild);
      clone.querySelector('#mqOdoTrail')?.setAttribute('stroke','#111');
      const xml  = new XMLSerializer().serializeToString(clone);
      const url  = URL.createObjectURL(new Blob([xml],{type:'image/svg+xml'}));
      Object.assign(document.createElement('a'),
        {href:url, download:`maqueen-drawing-${new Date().toISOString().slice(0,10)}.svg`}
      ).click();
      setTimeout(() => URL.revokeObjectURL(url), 5000);
    }
    ...
    function previewPoints() {
      const h = homePos();
      let px = h.x, py = h.y, ang = 0;
      const pts = [{x:px,y:py}];
      commands.forEach(({type,val}) => {
        const r = ang * Math.PI / 180;
        if      (type==='fwd')  { px += val*Math.sin(r); py += val*Math.cos(r); pts.push({x:px,y:py}); }
        else if (type==='back') { px -= val*Math.sin(r); py -= val*Math.cos(r); pts.push({x:px,y:py}); }
        else if (type==='right') ang = (ang + val) % 360;
        else if (type==='left')  ang = ((ang - val) % 360 + 360) % 360;
      });
      return pts;
    }

    function renderPreview() {
      const el = document.getElementById('mqTurtlePreview');
      if (!el) return;
      if (!enabled || commands.length === 0) { el.setAttribute('points',''); return; }
      const s = odoScale();
      el.setAttribute('points',
        previewPoints().map(p => `${(p.x*s).toFixed(1)},${(-p.y*s).toFixed(1)}`).join(' '));
    }

    function renderCmdList() {
      const ICON  = {fwd:'↑',back:'↓',left:'↺',right:'↻'};
      const UNIT  = {fwd:'cm',back:'cm',left:'°',right:'°'};
      const COLOR = {fwd:'#4ade80',back:'#f87171',left:'#38bdf8',right:'#fb923c'};
      const empty = penMode === 'draw' ? '— tap the map —' : '— add commands —';
      const html  = commands.length
        ? commands.map((c,i) =>
            `<span class="mq-tcmd-chip" data-i="${i}" style="--cc:${COLOR[c.type]}">`+
            `<sup class="mq-tcmd-num">${i+1}</sup>${ICON[c.type]} ${c.val}${UNIT[c.type]}</span>`).join('')
        : `<span class="mq-tcmd-empty">${empty}</span>`;
      // Update both panels (turtle + draw)
      ['mqTurtleCmds','mqDrawCmds'].forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        el.innerHTML = html;
        el.querySelectorAll('.mq-tcmd-chip').forEach(chip =>
          chip.addEventListener('click', () => {
            commands.splice(+chip.dataset.i, 1);
            renderCmdList(); renderPreview();
          }));
      });
    }

    function addCmd() {
      const v = parseFloat(document.getElementById('mqTurtleVal')?.value);
      if (!isNaN(v) && v > 0) { commands.push({type:selCmd, val:v}); renderCmdList(); renderPreview(); }
    }

    function stopExec() {
      execTimers.forEach(clearTimeout); execTimers = []; executing = false;
      try { fireDrive(0,0); } catch {}
      updateRunBtn();
    }

    function runCommands() {
      if (executing) { stopExec(); return; }
      if (!commands.length) return;
      executing = true; updateRunBtn();
      const h = homePos();
      if (typeof mqOdometry !== 'undefined') mqOdometry.reset(h.x, h.y);
      let delay = 0;
      commands.forEach(({type,val}) => {
        const dur = (type==='fwd'||type==='back') ? (val/CM_PER_SEC)*1000 : (val/DEG_PER_SEC)*1000;
        execTimers.push(setTimeout(() => {
          if (!executing) return;
          if      (type==='fwd')   fireDrive( SPEED,  SPEED);
          else if (type==='back')  fireDrive(-SPEED, -SPEED);
          else if (type==='right') fireDrive( SPEED, -SPEED);
          else if (type==='left')  fireDrive(-SPEED,  SPEED);
        }, delay));
        delay += dur;
        execTimers.push(setTimeout(() => { if (executing) fireDrive(0,0); }, delay));
        delay += 80;
      });
      execTimers.push(setTimeout(() => { executing = false; updateRunBtn(); }, delay));
    }

    function updateRunBtn() {
      const b = document.getElementById('mqTurtleRun');
      if (b) { b.textContent = executing ? '⏹ Stop' : '▶ Run'; b.classList.toggle('mq-turtle-running', executing); }
    }

    // ---- A: draw-then-drive --------------------------------------
    function isDrawMode() { return enabled && penMode === 'draw'; }
    ...
    function renderWaypoints() {
      const g = document.getElementById('mqWaypointDots');
      if (!g) return;
      if (!enabled) { g.innerHTML=''; return; }
      const s = odoScale();
      g.innerHTML = waypoints.map((wp,i) => {
        const sx = (wp.x*s).toFixed(1), sy = (-wp.y*s).toFixed(1);
        const prev = i>0 ? waypoints[i-1] : null;
        const line = prev
          ? `<line x1="${(prev.x*s).toFixed(1)}" y1="${(-prev.y*s).toFixed(1)}" x2="${sx}" y2="${sy}" stroke="#facc15" stroke-width="0.8" stroke-dasharray="3,2" opacity="0.55"/>`
          : '';
        const color = i===0 ? '#4ade80' : '#facc15';
        return line + `<circle cx="${sx}" cy="${sy}" r="3.5" fill="${color}" stroke="white" stroke-width="0.8" opacity="0.9"/>`;
      }).join('');
    }

    function waypointsToCommands() {
      if (waypoints.length < 1) { commands=[]; renderCmdList(); renderPreview(); return; }
      const h = homePos();
      let px=h.x, py=h.y, heading=0;
      commands = [];
      waypoints.forEach(wp => {
        const dx=wp.x-px, dy=wp.y-py, dist=Math.sqrt(dx*dx+dy*dy);
        if (dist < 0.5) return;
        const tgt = Math.atan2(dx,dy)*180/Math.PI;
        let turn = ((tgt - heading + 540) % 360) - 180;
        if (Math.abs(turn) > 2)
          commands.push({type: turn>0 ? 'right' : 'left', val: Math.round(Math.abs(turn))});
        commands.push({type:'fwd', val:Math.round(dist)});
        px=wp.x; py=wp.y; heading=tgt;
      });
      renderCmdList(); renderPreview();
    }
    ...
    function toggle() {
      enabled = !enabled;
      document.getElementById('mqOdoSvg')?.classList.toggle('mq-pen-mode', enabled);
      const panel = document.getElementById('mqPenPanel');
      if (panel) panel.style.display = enabled ? 'flex' : 'none';
      document.getElementById('mqPenToggle')?.classList.toggle('mq-pen-active', enabled);
      if (!enabled) {
        waypoints = [];
        document.getElementById('mqWaypointDots')?.setAttribute && (document.getElementById('mqWaypointDots').innerHTML = '');
        document.getElementById('mqTurtlePreview')?.setAttribute('points','');
      }
    }

    function switchMode(m) {
      penMode = m;
      document.getElementById('mqPenTurtleUI')?.style.setProperty('display', m==='turtle' ? 'flex':'none');
      document.getElementById('mqPenDrawUI')?.style.setProperty('display',   m==='draw'   ? 'flex':'none');
      document.getElementById('mqPenModeTurtle')?.classList.toggle('mq-pen-tab-active', m==='turtle');
      document.getElementById('mqPenModeDraw')?.classList.toggle('mq-pen-tab-active',   m==='draw');
      if (m !== 'draw') { waypoints=[]; renderWaypoints(); }
    }

    function init() {
      document.getElementById('mqPenToggle')?.addEventListener('click', toggle);
      document.getElementById('mqPenModeTurtle')?.addEventListener('click', () => switchMode('turtle'));
      document.getElementById('mqPenModeDraw')?.addEventListener('click',   () => switchMode('draw'));
      document.getElementById('mqPenExport')?.addEventListener('click', exportSvg);
      document.querySelectorAll('.mq-turtle-btn').forEach(b => b.addEventListener('click', () => {
        selCmd = b.dataset.cmd;
        document.querySelectorAll('.mq-turtle-btn').forEach(x => x.classList.remove('mq-turtle-btn-sel'));
        b.classList.add('mq-turtle-btn-sel');
        const unit = document.getElementById('mqTurtleUnit');
        if (unit) unit.textContent = (selCmd==='left'||selCmd==='right') ? '°' : 'cm';
      }));
      const valInput = document.getElementById('mqTurtleVal');
      document.getElementById('mqTurtleAdd')?.addEventListener('click', addCmd);
      valInput?.addEventListener('keydown', e => e.key==='Enter' && addCmd());
      document.getElementById('mqTurtleRun')?.addEventListener('click', runCommands);
      document.getElementById('mqDrawRun')?.addEventListener('click', runCommands);
      document.getElementById('mqTurtleClear')?.addEventListener('click', () => {
        commands=[]; waypoints=[]; renderCmdList(); renderPreview(); renderWaypoints(); stopExec();
      });
      document.getElementById('mqDrawClear')?.addEventListener('click', () => {
        waypoints=[]; renderWaypoints(); waypointsToCommands();
      });
    }

    return { toggle, init, isDrawMode, isEnabled:()=>enabled, addWaypoint, renderPreview, renderWaypoints };
  })();
  ...
  function initMaqueenSubTabs() {
    ...
    // Servo part: reset both servos to 0°.
    // Drive part: reset odometry heading to 0 (robot is now facing forward).
    anatomyParts.forEach(g => g.addEventListener('click', () => {
      show(g.dataset.mqTarget);
      if (g.dataset.mqTarget === 'servos') {
        try { setAngle(1, 0); } catch {}
        try { setAngle(2, 0); } catch {}
      }
    }));
